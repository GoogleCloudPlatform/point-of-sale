...
class InventoryIntegrationTest extends AbstractContainerDatabaseTest {
    ...
    static void registerTestProperties(DynamicPropertyRegistry registry) {
        registry.add("spring.datasource.url",
                () -> String.format("jdbc:mysql://%s:%d/pos_db", toxiproxy.getHost(), toxiproxy.getFirstMappedPort() + 1));
    ...
    void initializeTestProxy() throws IOException {
        if (toxiproxyClient == null) {
            ...
            apiProxy = toxiproxyClient.createProxy("apiProxy",
                    "0.0.0.0:" + toxiproxyPort,
                    hostRunningTheAppEndpoint.getHostAddress() + ":" + localServerPort);
            proxyHostAndPort = String.format("http://%s:%d", toxiproxy.getHost(), toxiproxy.getMappedPort(toxiproxyPort));
        }
    }

    @Test
    void testConnectionCut() throws IOException {
        ResponseEntity<String> itemsBeforeToxic = this.testRestTemplate.getForEntity(proxyHostAndPort + "/items", String.class);
        ...
        assertThat(
                catchThrowable(() -> {
                    ResponseEntity<String> itemsDuringToxic = this.testRestTemplate.getForEntity(proxyHostAndPort + "/items", String.class);
                })
        )
        ...
        ResponseEntity<String> itemsAfterToxic = this.testRestTemplate.getForEntity(proxyHostAndPort + "/items", String.class);
        assertThat(
